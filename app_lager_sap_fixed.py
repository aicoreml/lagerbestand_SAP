    ...
    <body class="bg-gray-100">
        <div class="container mx-auto px-4 py-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div class="bg-white p-6 rounded-lg shadow-md">
                    <h2 class="text-xl font-bold mb-4">Lagerstatus</h2>
                    <div class="overflow-x-auto">  <!-- Remove height restriction -->
        ...
        <script>
                ...
                try {
                    ...
                    if (tableBody) {
                        ...
                        dashboardFieldMapping.forEach(mapping => {
                            ...
                            row.innerHTML = `
                                <td class="px-6 py-4 whitespace-nowrap">${mapping.dashboardField}</td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium">${mapping.sapTable}</td>
                                <td class="px-6 py-4 whitespace-nowrap">${mapping.sapField}</td>
                                <td class="px-6 py-4">${mapping.description}</td>
                            `;
            ...
            async function loadInventoryData(selectedCategory = null) {
                try {
                    ...
                    if (tableBody) {
                        ...
                        data.forEach(item => {
                            ...
                            row.innerHTML = `
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">${item.last_restock_date || 'N/A'}</td>
                    ...
                    const tableBody = document.getElementById('inventoryTableBody');
                    if (tableBody) {
                        // Fixed: Changed colspan from 6 to 7 to match the actual table structure
                ...
                try {
                    ...
                    if (tableBody) {
                        ...
                        forecasts.forEach(forecast => {
                            ...
                            row.innerHTML = `
                                <td class="px-6 py-4 whitespace-nowrap">${forecast.forecast_date}</td>
                                <td class="px-6 py-4 whitespace-nowrap">${forecast.product_id}</td>
                                <td class="px-6 py-4 whitespace-nowrap">${forecast.predicted_demand ? forecast.predicted_demand.toFixed(2) : 'N/A'}</td>
                                <td class="px-6 py-4 whitespace-nowrap">${forecast.confidence_level ? (forecast.confidence_level * 100).toFixed(1) + '%' : 'N/A'}</td>
                            `;
                            tableBody.appendChild(row);
                        });
                    }
                    ...
                    if (container) {
                        ...
                        Object.entries(productsByCategory).forEach(([category, products]) => {
                            ...
                            categoryHeader.innerHTML = `
                                ...
                                <input type="checkbox" id="cat_${category}" class="mr-2 category-checkbox">
                                <label for="cat_${category}" class="font-medium text-sm">${category}</label>
                                <span class="ml-auto text-xs text-gray-500">(${products.length} Produkte)</span>
                            `;
                            ...
                            products.forEach(item => {
                                ...
                                productDiv.innerHTML = `
                                    <input type="checkbox" id="product_${item.product_id}" value="${item.product_id}" class="mr-2 product-checkbox" data-category="${category}">
                                    <label for="product_${item.product_id}" class="text-sm">${item.product_id} - ${item.product_name}</label>
                                `;
